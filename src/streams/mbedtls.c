...
int mbedtls_certificate(git_cert **out, git_stream *stream)
{
	...
	// Backporting from HEAD: field rename
	...
	// Backporting from HEAD: field rename
	// Backporting from HEAD: incompatible pointer types (struct vs enum)
	*out = (git_cert *)&st->cert_info.cert_type;

	return 0;
}

// Backporting: proxies are not supported for streams
#if 0
...
int git_mbedtls_stream_new(git_stream **out, const char *host, const char *port)
{
	...
	// Backporting: proxy not supported for any streams
